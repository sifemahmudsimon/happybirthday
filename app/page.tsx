...
export default function Home() {
  ...
  useEffect(() => {
    ...
    asset.onload = asset.onerror = () => { loaded = true; };
    ...
    return () => { clearInterval(timer); asset.onload = asset.onerror = null; };
  }, []);
  ...
  function throughPortal() {
    if (portal) return;
    setPortal(true); setBurst(v => v + 1);
    portalTimer.current = setTimeout(() => { go(1); setPortal(false); }, reduced ? 0 : 1000);
  }
  function showPhoto(index: number) { setFortune(null); setPhoto(index); dialog.current?.showModal(); }
  function showFortune(index: number) { setPhoto(null); setFortune(index); dialog.current?.showModal(); }
  function connect(name: string) { setFriends(current => current.includes(name) ? current : [...current, name]); }
  function stir(e: PointerEvent<HTMLButtonElement>) {
    if (!lastPointer.current) return;
    const distance = Math.hypot(e.clientX-lastPointer.current.x, e.clientY-lastPointer.current.y);
  ...
  return <>
    ...
    {!opened && <div className="prologue" role="dialog" aria-modal="true" aria-labelledby="prologue-title">
      <div className="prologue-art" /><div className="prologue-shade" />
      <span className="micro prologue-top">A SMALL GIFT FROM SIMON & OLIVE</span>
      <div className="prologue-center"><div className="seed-orbits"><i/><i/><i/><Spark/></div><p className="micro">SEPTEMBER 03 · A WORLD MADE FOR YOU</p><h1 id="prologue-title">Something lovely<br />is about to <em>happen.</em></h1><div className="loading-line" role="progressbar" aria-label="Preparing Priya’s universe" aria-valuenow={progress} aria-valuemin={0} aria-valuemax={100}><span style={{width:`${progress}%`}}/></div><span className="load-number">{String(progress).padStart(2,"0")} <i>/ 100</i></span><button className="enter-button" disabled={progress<100} onClick={enter}>{progress<100 ? "Gathering a little magic" : "Let the magic happen"}<span>↗</span></button><p className="sound-note">Sound on. Ordinary world off.</p></div>
      <button className="motion-choice" onClick={() => setMotion(reduced ? "full" : "calm")}>{reduced ? "Gentle motion enabled · choose full experience" : "Full experience · prefer gentle motion?"}</button>
    </div>}

    <div ref={stage} className={`universe-stage ${opened ? "universe-open" : ""} ${portal ? "travelling-portal" : ""}`} inert={!opened}>
      <div className="world-backdrop"/><div className="world-tint"/><div className="world-grain"/>
      ...
      <main className="scene-world" aria-label="Priya’s interactive birthday story">
        <section className="world-scene opening-scene" aria-label="A little universe" style={{"--local":0} as CSSProperties}>
          ...
          <div className="portal-stage"><div className="portal-halo"/><span className="portal-orbit-label">EARTH SIGN · SOFT HEART · QUIET MAGIC ·</span><button className="virgo-doorway" onClick={throughPortal} aria-label="Enter through the Virgo doorway"><div className="doorway-inner"><span className="doorway-date">III · IX</span><CardArt kind="virgo"/><span className="doorway-title">The Virgo</span><span className="doorway-invite">YOUR STORY IS WAITING <b>↗</b></span></div></button><div className="satellite-card satellite-left"><button onClick={()=>showFortune(0)} aria-label="Reveal the dreamer fortune"><CardArt kind="moon"/><span>The dreamer</span></button></div><div className="satellite-card satellite-right"><button onClick={()=>showFortune(3)} aria-label="Reveal the heart fortune"><CardArt kind="heart"/><span>The heart</span></button></div><span className="portal-handwritten script">a doorway, just for you ↗</span></div>
          <div className="opening-corner"><span>♍︎</span><p>ONE BEAUTIFUL SOUL.<br/>AN ENTIRE UNIVERSE OF LOVE.</p></div>
        </section>
        ...
        <section className="world-scene connection-scene" aria-label="An invisible thread">
          ...
          <div className="constellation-stage"><Ribbon connected={friends.length/3}/>{["Simon","Priya","Olive"].map((name,i)=><button key={name} onClick={()=>connect(name)} className={`friend-orb friend-${i} ${friends.includes(name) ? "connected" : ""}`} aria-pressed={friends.includes(name)} aria-label={`Connect ${name}`}><span className="friend-orb-art"><CardArt kind={["sun","virgo","moon"][i]}/></span><b>{name}</b><span>{friends.includes(name) ? "CONNECTED BY A LITTLE MAGIC" : "TAP TO CONNECT"}</span></button>)}</div>
          <div className="connection-caption" aria-live="polite"><p>{friends.length===3 ? "And just like that: best buddies. A little constellation of our own." : "First we talked. Then we shared. Somewhere along the way, colleagues became our people."}</p><span className="script">{friends.length===3 ? "three people. so many stories. ♡" : "tap our stars. see what brings us together."}</span></div>
        </section>
